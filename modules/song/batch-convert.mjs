...
for (const file of xmlFiles) {
	...
	const r = convertFile(file, { overwrite });
	if (!r) continue;
	if (r.status === "wrote") {
		results.wrote.push(file);
		console.log(`wrote (${dialect}): ${relative(root, r.out)}`);
	} else {
